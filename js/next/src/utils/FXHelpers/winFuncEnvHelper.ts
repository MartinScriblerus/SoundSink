...
const attackTime: number = 25;
const releaseTime: number = 25;
const value: number = 0.5;

const winFuncEnvHelper = () => {
    return `
        ...
        // Noise nois 
        //SinOsc s2 => WinFuncEnv win1 => Gain gtest => dac;
        ...
        // playWindow(win1, ${attackTime}::ms, ${releaseTime}::ms);

        // // sets Blackman Envelope with a custom value
        // win1.setBlackman(${value});
        ...
        // playWindow(win1, ${attackTime}::ms, 100::ms);
        ...
        spork ~ loop(win1, 72::ms);
        loop(win2, 73::ms);

    `
};
export default winFuncEnvHelper;